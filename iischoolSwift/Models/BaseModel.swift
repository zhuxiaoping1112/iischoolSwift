...
struct InfoModel {
    ...
    init(dict: NSDictionary) {
        ...
        if let array = favUserArray {
            self.fav_users = array.map({ (favUser) -> String in
                "\(favUser)"
            })
        }
        ...
        self.fav = "\(String(describing: dict["fav"]))"
        self.up = "\(String(describing: dict["up"]!))"
        self.down = "\(String(describing: dict["down"]))"
        self.av = "\(String(describing: dict["av"]))"
    }
}
